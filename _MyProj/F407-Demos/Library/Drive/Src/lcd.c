...
void LCD_Init(void) {
    ...
    gpioConf(LCD_LIGHT_PIN, GPIO_Speed_100MHz, GPIO_Mode_OUT, GPIO_OType_PP,
             GPIO_PuPd_UP);
    gpioAFConf(GPIOD,
               GPIO_Pin_0 | GPIO_Pin_1 | GPIO_Pin_4 | GPIO_Pin_5 | GPIO_Pin_8 |
                   GPIO_Pin_9 | GPIO_Pin_10 | GPIO_Pin_14 | GPIO_Pin_15,
               GPIO_Speed_100MHz, GPIO_OType_PP, GPIO_PuPd_UP, GPIO_AF_FSMC);
    gpioAFConf(GPIOE,
               GPIO_Pin_7 | GPIO_Pin_8 | GPIO_Pin_9 | GPIO_Pin_10 | GPIO_Pin_11 |
                   GPIO_Pin_12 | GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15,
               GPIO_Speed_100MHz, GPIO_OType_PP, GPIO_PuPd_UP, GPIO_AF_FSMC);
    gpioAFConf(GPIOF, GPIO_Pin_12, GPIO_Speed_100MHz, GPIO_OType_PP,
               GPIO_PuPd_UP, GPIO_AF_FSMC);
    gpioAFConf(GPIOG, GPIO_Pin_12, GPIO_Speed_100MHz, GPIO_OType_PP,
               GPIO_PuPd_UP, GPIO_AF_FSMC);
    ...
    } else if (lcddev.id == 0x6804)  // 6804初始化
    {
        LCD_WR_REG(0X11);
        sysDelayMS(20);
        LCD_WR_REG(0XD0);  // VCI1  VCL  VGH  VGL DDVDH VREG1OUT power amplitude
                           // setting
        LCD_WR_DATA(0X07);
        LCD_WR_DATA(0X42);
        LCD_WR_DATA(0X1D);
    ...
    } else if (lcddev.id == 0X9331)    // OK |/|/|
    {
        ...
        sysDelayMS(200);               // Dis-charge capacitor power voltage
        LCD_WriteReg(0x0010, 0x1690);  // SAP, BT[3:0], AP, DSTB, SLP, STB
        LCD_WriteReg(0x0011, 0x0227);  // DC1[2:0], DC0[2:0], VC[2:0]
        sysDelayMS(50);                // Delay 50ms
    ...
    } else if (lcddev.id == 0x1505)  // OK
    {
        ...
        LCD_WriteReg(0x0007, 0x0000);
        sysDelayMS(50);
        LCD_WriteReg(0x0012, 0x011C);  // 0x011A   why need to set several
                                       // times?
        ...
        LCD_WriteReg(
            0x0011, 0x0217);  // reference voltage VC[2:0]   Vciout = 1.00*Vcivl
        LCD_WriteReg(0x0012, 0x011E);  // 0x011c  //Vreg1out = Vcilvl*1.80   is
                                       // it the same as Vgama1out ?
        LCD_WriteReg(
            0x0013,
            0x0F00);  // VDV[4:0]-->VCOM Amplitude VcomL = VcomH - Vcom Ampl
        LCD_WriteReg(0x002A, 0x0000);
        LCD_WriteReg(0x0029, 0x000A);  // 0x0001F  Vcomh = VCM1[4:0]*Vreg1out
                                       // gate source voltage??
