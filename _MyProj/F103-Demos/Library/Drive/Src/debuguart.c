...
int8_t debugRxBuff[DEBUG_MAX_LEN];  // 缓冲区
Func debugFun;                      // 调试串口响应函数
...
void debugInit(uint32_t baud) {
    gpioConf(DEBUG_PORT, DEBUG_T, GPIO_Speed_10MHz, GPIO_Mode_AF_PP);
    gpioConf(DEBUG_PORT, DEBUG_R, GPIO_Speed_10MHz, GPIO_Mode_IN_FLOATING);
#ifdef EN_DEBUG_RX
...
/**
 * @brief 清除调试串口 Flag 并且执行响应函数 (不使用shell时)
 *
**/
...
void print(const char *fmt, ...) {
    ...
    for (; *fmt != '\0'; fmt++) {
        ...
        switch (*fmt) {
            case 'd':
                putnum(va_arg_(ap, int), 10); // 十进制
                break;
            case 'x':
                putnum(va_arg_(ap, int), 16); // 十六进制
                break;
            case 'o':
                putnum(va_arg_(ap, int), 8);  // 八进制
                break;
            case 'c':
                putch(va_arg_(ap, char));     // 字符
                break;
            case 's':
